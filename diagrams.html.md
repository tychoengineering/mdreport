...
you can pass other values. See [The scale]() for details.
...
![A single node labeled Source](_static/diagrams/one-node-light.svg)![A single node labeled Source](_static/diagrams/one-node-dark.svg)
...
![A Source node with an arrow to a Store node](_static/diagrams/two-nodes-light.svg)![A Source node with an arrow to a Store node](_static/diagrams/two-nodes-dark.svg)
...
![Source, Transform, and Output artifact nodes connected left to right](_static/diagrams/three-roles-light.svg)![Source, Transform, and Output artifact nodes connected left to right](_static/diagrams/three-roles-dark.svg)
...
![The three-node flow with a dashed retry path and a legend](_static/diagrams/with-legend-light.svg)![The three-node flow with a dashed retry path and a legend](_static/diagrams/with-legend-dark.svg)
...
![A titled panel around the three-node flow, above a legend](_static/diagrams/grouped-light.svg)![A titled panel around the three-node flow, above a legend](_static/diagrams/grouped-dark.svg)
...
![A record reaching a schema check that accepts or quarantines it](_static/diagrams/branching-light.svg)![A record reaching a schema check that accepts or quarantines it](_static/diagrams/branching-dark.svg)
...
![A record field list feeding a batch that splits between publish and cache](_static/diagrams/shapes-light.svg)![A record field list feeding a batch that splits between publish and cache](_static/diagrams/shapes-dark.svg)
...
![Extract and Publish lanes inside a private network boundary](_static/diagrams/lanes-light.svg)![Extract and Publish lanes inside a private network boundary](_static/diagrams/lanes-dark.svg)
...
![Four nodes with right-side icons: Warehouse tables, Spreadsheets, Runbooks, and Checked record](_static/diagrams/icons-light.svg)![Four nodes with right-side icons: Warehouse tables, Spreadsheets, Runbooks, and Checked record](_static/diagrams/icons-dark.svg)
...
![The two-node flow in brand green and purple](_static/diagrams/branded-light.svg)![The two-node flow in brand green and purple](_static/diagrams/branded-dark.svg)
...
![A three-node flow whose middle node carries a thicker orange-to-green gradient border](_static/diagrams/bold-nodes-light.svg)![A three-node flow whose middle node carries a thicker orange-to-green gradient border](_static/diagrams/bold-nodes-dark.svg)
...
![A single node on a gradient canvas inside a rounded gradient frame](_static/diagrams/framed-light.svg)![A single node on a gradient canvas inside a rounded gradient frame](_static/diagrams/framed-dark.svg)
...
![A producer and consumer joined through an ellipse labeled Queue](_static/diagrams/custom-element-light.svg)![A producer and consumer joined through an ellipse labeled Queue](_static/diagrams/custom-element-dark.svg)
...
rejected, but unknown nonblank strings pass through as literal CSS paint. See the [API reference](api-reference.html.md)
